...
/**
 * En un polideportivo se imparten clases de natación	
 * 5 días a la semana,
 * 4 horas al día.	
 * Se desea	controlar el número	de niños que acuden	a cada hora cada día,
 * sabiendo	que	puede haber	un mínimo de 10	y un máximo	de 20 niños	por	clase.
 * 
 * Diseña una estructura de	datos que permita almacenar	dichos valores e introduce datos	
 * aleatorios en cada posición.
 * Realiza un programa	que	permita	realizar	las	siguientes	operaciones:
 *  - Mostrar los valores almacenados y	el número total de alumnos diarios siguiendo la siguiente propuesta:
 * o Lunes:	    15	19	20	12
 ...
 * • Mostrar el/los	día/s y la/s hora/s en que hay un número determinado de	alumnos	introducido	por	el usuario.
 * @author manavas
 *
 */
...
public class ClasesNatacion {
	...
	private static final int PRIMERA_HORA = 0;
	private static final int SEGUNDA_HORA = 1;
	private static final int TERCERA_HORA = 2;
	private static final int CUARTA_HORA = 3;
		...
		for (int i = 0; i < clasesNatacion.length; i++) {
			for (int j = 0; j < clasesNatacion[i].length; j++) {
                if (clasesNatacion[i][j] == numero) {
                	System.out.println("\nEncontrado numero: " + numero + " el dia " + DIAS_SEMANA.get(i) + " a la hora " + (j+1));
                }
            }
		}
	}
